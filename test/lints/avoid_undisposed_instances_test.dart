...
void main() {
  group('AvoidUndisposedInstances', () {
    test('should trigger lint when disposable instance is not disposed', () async {
      ...
      final shouldTriggerLintErrors = lines
          .where((line) =>
              line.contains('avoid_undisposed_instances/should_trigger_lint.dart') &&
              line.contains('unfulfilled_expect_lint'))
          .toList();

      final shouldNotTriggerLintIssues = lines
          .where((line) =>
              line.contains('avoid_undisposed_instances/should_not_trigger_lint.dart') &&
              line.contains('avoid_undisposed_instances') &&
              !line.contains('unfulfilled_expect_lint'))
          .toList();
