...
void main() {
  group('DisposeFields', () {
    test('should trigger lint when disposable fields are not disposed', () async {
      ...
      final shouldTriggerLintErrors = lines
          .where((line) =>
              line.contains('dispose_fields/should_trigger_lint.dart') &&
              line.contains('unfulfilled_expect_lint'))
          .toList();

      final shouldNotTriggerLintIssues = lines
          .where((line) =>
              line.contains('dispose_fields/should_not_trigger_lint.dart') &&
              line.contains('dispose_fields') &&
              !line.contains('unfulfilled_expect_lint'))
          .toList();
